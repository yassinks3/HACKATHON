...
async function loadInitialData() {
    ...
    //Exists function when supabaseClient is not available
    ...
    try {
        ...
        //calls the function to refresh live profiles from Supabase
...
//fetches profiles and skills from supabase and joins them in memory
...
async function acceptMatch(candidateId) {
    ...
    // Update the accepted count badge in the UI
    document.getElementById('accepted-count-badge').textContent = state.acceptedCandidates.length;
    await renderCandidateMatches();
    renderAcceptedSwaps();
    alert('Match Accepted! You can now chat in "My Swaps".');
}
// Passes a candidate match, saves the decision to Supabase
async function passMatch(candidateId) {
    ...
    // Refreshes live profiles, and compute candidate matches
    await renderCandidateMatches();
}
...
// Renders the list of accepted swaps in the "My Swaps" section
function renderAcceptedSwaps() {
    ...
    //If acceptedProfiles is empty, display a message indicating no accepted swaps yet
    ...
    //If completedSwaps is not defined in state, initialize it as an empty array
    if (!state.completedSwaps) state.completedSwaps = [];

    //Checks if each accepted profile is marked as completed and sorts them accordingly, with completed swaps at the bottom 
    ...
    //Generates the HTML for each accepted swap card, including the candidate's profile info, completion status, and action buttons
...
//Toggles a swap status to "Skill Learned", and moves card to the bottom
async function toggleSkillLearned(candidateId) {
    ...
    // Re-render the accepted swaps list to reflect the updated completion status
    renderAcceptedSwaps();
}
